...
struct FoodLibrary {
    static let all: [FoodLibraryItem] = [
        ...
        FoodLibraryItem(name: "Sweet Potato",          category: .carbs,      servingSize: 173, servingUnit: "1 medium",   caloriesPerServing: 161, proteinPerServing: 4,   carbsPerServing: 37,  fatPerServing: 0,   mealTypes: ["Lunch","Dinner"]),
        ...
        FoodLibraryItem(name: "Walnuts",               category: .fats,       servingSize: 28,  servingUnit: "1 oz",       caloriesPerServing: 185, proteinPerServing: 4,   carbsPerServing: 4,   fatPerServing: 18,  mealTypes: ["Snack"]),
    ...
    static func byCategory(_ category: FoodLibraryItem.FoodCategory) -> [FoodLibraryItem] { all.filter { $0.category == category } }
    static func forMealType(_ mealType: String) -> [FoodLibraryItem] { all.filter { $0.mealTypes.contains(mealType) } }
}
...
struct SelectedFood: Identifiable {
    ...
    var servingLabel: String {
        servings == floor(servings)
            ? "\(Int(servings))x \(food.servingUnit)"
            : String(format: "%.1fx \(food.servingUnit)", servings)
    }
}
...
struct MealPlanBuilderView: View {
    ...
    var canSave: Bool { !title.isEmpty && !meals.isEmpty }
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        Text("DAILY TARGETS").font(.system(size: 10, weight: .bold)).tracking(1.2).foregroundColor(.tmGold)
                        if !meals.isEmpty {
                            HStack(spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        HStack {
                            VStack(alignment: .leading, spacing: 2) {
                                Text("MEALS").font(.system(size: 10, weight: .bold)).tracking(1.2).foregroundColor(.tmGold)
                                if !meals.isEmpty { Text("\(meals.count) meals • \(totalCalories) cal").font(.caption2).foregroundColor(.white.opacity(0.4)) }
                            }
                            Spacer()
                            Button(action: { showingAddMeal = true }) {
                                HStack(spacing: 6) { Image(systemName: "plus.circle.fill").font(.caption); Text("Add Meal").font(.caption).fontWeight(.semibold) }
                        ...
                        if meals.isEmpty {
                            Button(action: { showingAddMeal = true }) {
                                VStack(spacing: 12) {
                                    Image(systemName: "fork.knife").font(.system(size: 32)).foregroundColor(.tmGold.opacity(0.4))
                                    Text("Tap to build your meal plan").font(.subheadline).foregroundColor(.white.opacity(0.35))
                                    Text("50+ foods with adjustable serving sizes").font(.caption).foregroundColor(.white.opacity(0.25))
                                }
                            ...
                            ForEach(Array(meals.enumerated()), id: \.element.id) { i, meal in
                                BuiltMealCard(meal: meal,
                                    ...
                                    onEdit: { updated in if let idx = meals.firstIndex(where: { $0.id == meal.id }) { meals[idx] = updated } })
                            }
                        }
                    }
                    ...
                    Button(action: {
                        ...
                        let rows = meals.map { m -> MealItem in
                            MealItem(id: UUID(), mealType: m.mealType, name: m.name,
                                     calories: m.totalCalories, protein: m.totalProtein,
                                     carbs: m.totalCarbs, fat: m.totalFat,
                                     notes: m.foods.map { "\($0.servingLabel) \($0.food.name)" }.joined(separator: ", "))
                        }
                        let plan = MealPlanRow(id: UUID(),
                            trainerId: UUID(uuidString: trainerId) ?? UUID(),
                            clientId: UUID(uuidString: clientId) ?? UUID(),
                            title: title, description: description, meals: rows,
                            dailyCalories: dailyCalories, proteinG: proteinTarget,
                            carbsG: carbsTarget, fatG: fatTarget,
                            weekStart: nil, isActive: false, createdAt: Date())
                        TrainerLocalCache.shared.saveMealPlanAsTemplate(plan)
                    }) {
                        HStack(spacing: 6) {
                    ...
                    Button(action: savePlan) {
                        HStack(spacing: 8) {
                            if isSaving { ProgressView().tint(.black) } else {
                                ...
                                Text("ASSIGN TO \(clientName.uppercased())").font(.system(size: 14, weight: .heavy)).tracking(0.5)
                            }
                        }
                        .foregroundColor(canSave ? .black : .white.opacity(0.4))
                        .frame(maxWidth: .infinity).frame(height: 54)
                        .background(RoundedRectangle(cornerRadius: 27).fill(canSave ? Color.tmGold : Color.white.opacity(0.08)))
                    }
        ...
        .sheet(isPresented: $showingAddMeal) { MealBuilderSheet { newMeal in meals.append(newMeal) } }
    }

    private func savePlan() {
        print("💾 savePlan — clientId: '\(clientId)' trainerId: '\(trainerId)'")
        guard canSave else { return }

        guard let tUUID = UUID(uuidString: trainerId) else {
            saveError = "Invalid trainer ID: \(trainerId)"; return
        }
        guard let cUUID = UUID(uuidString: clientId) else {
            saveError = "Invalid client ID: \(clientId)"; return
        }

        isSaving = true
        saveError = nil

        let mealItems = meals.map { m -> MealItem in
            MealItem(id: UUID(), mealType: m.mealType, name: m.name,
                     calories: m.totalCalories, protein: m.totalProtein,
                     carbs: m.totalCarbs, fat: m.totalFat,
                     notes: m.foods.map { "\($0.servingLabel) \($0.food.name)" }.joined(separator: ", "))
        ...
        let plan = MealPlanRow(
            ...
            weekStart: nil, isActive: true, createdAt: Date()
        )

        Task {
            do {
                try await SBMealPlanStore.shared.create(plan)
                print("✅ Meal plan saved — clientId: \(cUUID)")
    ...
    private func macroActualCell(_ actual: Int, _ target: Int, _ label: String, _ color: Color) -> some View {
        ...
        return VStack(spacing: 4) {
            Text("\(actual)").font(.system(size: 14, weight: .black)).foregroundColor(over ? .orange : color)
            ...
            GeometryReader { geo in ZStack(alignment: .leading) { RoundedRectangle(cornerRadius: 2).fill(Color.white.opacity(0.1)); RoundedRectangle(cornerRadius: 2).fill(over ? Color.orange : color).frame(width: geo.size.width * pct) } }.frame(height: 3)
    ...
    private func macroStepper(_ label: String, value: Binding<Int>, step: Int, color: Color) -> some View {
        HStack {
            Text(label).font(.system(size: 12, weight: .semibold)).foregroundColor(.white.opacity(0.6)); Spacer()
            Button(action: { if value.wrappedValue > step { value.wrappedValue -= step } }) { Image(systemName: "minus.circle.fill").foregroundColor(color).font(.title3) }
            Text("\(value.wrappedValue)").font(.system(size: 18, weight: .black)).foregroundColor(color).frame(minWidth: 55)
            Button(action: { value.wrappedValue += step }) { Image(systemName: "plus.circle.fill").foregroundColor(color).font(.title3) }
    ...
    private func macroDoubleStepper(_ label: String, value: Binding<Double>, step: Double, color: Color) -> some View {
        VStack(spacing: 6) {
            ...
            HStack(spacing: 4) {
                Button(action: { if value.wrappedValue > step { value.wrappedValue -= step } }) { Image(systemName: "minus.circle.fill").foregroundColor(color).font(.caption) }
                Text("\(Int(value.wrappedValue))").font(.system(size: 15, weight: .black)).foregroundColor(.white).frame(minWidth: 30)
                Button(action: { value.wrappedValue += step }) { Image(systemName: "plus.circle.fill").foregroundColor(color).font(.caption) }
            }
        }.frame(maxWidth: .infinity).padding(10).background(RoundedRectangle(cornerRadius: 10).fill(Color.white.opacity(0.05)))
    }
}
...
struct BuiltMealCard: View {
    let meal: BuiltMeal; let onDelete: () -> Void; let onEdit: (BuiltMeal) -> Void
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    Text(meal.mealType).font(.system(size: 10, weight: .bold)).foregroundColor(.tmGold)
                    Text(meal.name).font(.system(size: 14, weight: .semibold)).foregroundColor(.white)
                }
                Spacer()
                HStack(spacing: 8) {
                    Button(action: { showingEdit = true }) { Image(systemName: "pencil").font(.caption).foregroundColor(.tmGold).padding(6) }
            ...
            }.padding(.vertical, 8).background(RoundedRectangle(cornerRadius: 8).fill(Color.white.opacity(0.04)))
            VStack(alignment: .leading, spacing: 4) {
                ForEach(meal.foods) { sf in
                    HStack(spacing: 6) {
                        Circle().fill(sf.food.category.color).frame(width: 6, height: 6)
                        Text(sf.food.name).font(.caption).foregroundColor(.white.opacity(0.8)); Spacer()
        ...
        .sheet(isPresented: $showingEdit) { MealBuilderSheet(existingMeal: meal) { updated in onEdit(updated) } }
    }

    private func mealMacro(_ value: String, _ label: String, _ color: Color) -> some View {
        VStack(spacing: 2) { Text(value).font(.system(size: 13, weight: .black)).foregroundColor(color); Text(label).font(.system(size: 9)).foregroundColor(.white.opacity(0.35)) }.frame(maxWidth: .infinity)
    }
}
...
struct MealBuilderSheet: View {
    ...
    init(existingMeal: BuiltMeal? = nil, onSave: @escaping (BuiltMeal) -> Void) {
        self.existingMeal = existingMeal; self.onSave = onSave
    ...
    var canSave: Bool { !mealName.isEmpty && !selectedFoods.isEmpty }
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("MEAL TYPE").font(.system(size: 10, weight: .bold)).tracking(1.2).foregroundColor(.tmGold)
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 8) {
                                    ForEach(mealTypes, id: \.self) { type in
                                        Button(action: { mealType = type; if mealName.isEmpty { mealName = type } }) {
                                            Text(type).font(.system(size: 12, weight: .semibold))
                                                .foregroundColor(mealType == type ? .black : .white.opacity(0.6))
                                                .padding(.horizontal, 14).padding(.vertical, 8)
                                                .background(Capsule().fill(mealType == type ? Color.tmGold : Color.white.opacity(0.08)))
                                        }.buttonStyle(.plain)
                                    }
                                }
                            }
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            Text("MEAL NAME").font(.system(size: 10, weight: .bold)).tracking(1.2).foregroundColor(.tmGold)
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            HStack {
                                Text("FOODS").font(.system(size: 10, weight: .bold)).tracking(1.2).foregroundColor(.tmGold); Spacer()
                                Button(action: { showingLibrary = true }) {
                                    HStack(spacing: 5) { Image(systemName: "plus.circle.fill").font(.caption); Text("Browse Foods").font(.caption).fontWeight(.semibold) }
                                        .foregroundColor(.black).padding(.horizontal, 10).padding(.vertical, 5).background(Capsule().fill(Color.tmGold))
                                }
                            }
                            if selectedFoods.isEmpty {
                                Button(action: { showingLibrary = true }) {
                                    HStack(spacing: 10) { Image(systemName: "fork.knife").foregroundColor(.tmGold.opacity(0.5)); Text("Browse 50+ foods with adjustable servings").font(.caption).foregroundColor(.white.opacity(0.35)) }
                                        .frame(maxWidth: .infinity).padding(14)
                                        .background(RoundedRectangle(cornerRadius: 12).fill(Color.white.opacity(0.03)).overlay(RoundedRectangle(cornerRadius: 12).stroke(Color.tmGold.opacity(0.15), style: StrokeStyle(lineWidth: 1, dash: [5, 4]))))
                                }.buttonStyle(.plain)
                            } else {
                                ForEach(Array(selectedFoods.enumerated()), id: \.element.id) { i, sf in
                                    SelectedFoodRow(selectedFood: sf, onServingsChanged: { selectedFoods[i].servings = $0 }, onRemove: { selectedFoods.remove(at: i) })
                                }
                            }
                        }
                        Button(action: { let meal = BuiltMeal(mealType: mealType, name: mealName, foods: selectedFoods); onSave(meal); dismiss() }) {
                            Text(existingMeal != nil ? "UPDATE MEAL" : "ADD TO PLAN")
                                .font(.system(size: 15, weight: .heavy)).tracking(0.5)
                                .foregroundColor(canSave ? .black : .white.opacity(0.4))
                                .frame(maxWidth: .infinity).frame(height: 52)
                                .background(RoundedRectangle(cornerRadius: 26).fill(canSave ? Color.tmGold : Color.white.opacity(0.08)))
                        }.disabled(!canSave)
                    }.padding(16)
                }
            }
            .navigationTitle(existingMeal != nil ? "Edit Meal" : "Build Meal").navigationBarTitleDisplayMode(.inline)
            .toolbarBackground(Color.black, for: .navigationBar).toolbarBackground(.visible, for: .navigationBar).toolbarColorScheme(.dark, for: .navigationBar)
            .toolbar { ToolbarItem(placement: .navigationBarLeading) { Button("Cancel") { dismiss() }.foregroundColor(.tmGold) } }
            .sheet(isPresented: $showingLibrary) {
                FoodLibraryBrowser(mealTypeFilter: mealType) { food in
                    if let i = selectedFoods.firstIndex(where: { $0.food.id == food.id }) { selectedFoods[i].servings += 1 }
                    else { selectedFoods.append(SelectedFood(food: food, servings: 1.0)) }
                }
            }
        }
    }
    private func summaryMacro(_ value: String, _ label: String, _ color: Color) -> some View {
        VStack(spacing: 2) { Text(value).font(.system(size: 15, weight: .black)).foregroundColor(color); Text(label).font(.system(size: 9)).foregroundColor(.white.opacity(0.4)) }.frame(maxWidth: .infinity)
    }
}
...
struct SelectedFoodRow: View {
    let selectedFood: SelectedFood; let onServingsChanged: (Double) -> Void; let onRemove: () -> Void
    var body: some View {
        HStack(spacing: 10) {
            ZStack { RoundedRectangle(cornerRadius: 8).fill(selectedFood.food.category.color.opacity(0.15)).frame(width: 36, height: 36); Image(systemName: selectedFood.food.category.icon).font(.system(size: 14)).foregroundColor(selectedFood.food.category.color) }
            VStack(alignment: .leading, spacing: 3) {
                Text(selectedFood.food.name).font(.system(size: 13, weight: .semibold)).foregroundColor(.white)
                Text("\(selectedFood.calories) cal • P:\(String(format: "%.0f", selectedFood.protein))g C:\(String(format: "%.0f", selectedFood.carbs))g F:\(String(format: "%.0f", selectedFood.fat))g").font(.caption2).foregroundColor(.white.opacity(0.45))
            }
            Spacer()
            HStack(spacing: 6) {
                Button(action: { onServingsChanged(max(0.5, selectedFood.servings - 0.5)) }) { Image(systemName: "minus.circle.fill").font(.system(size: 20)).foregroundColor(.tmGold.opacity(0.8)) }
                VStack(spacing: 1) {
                    Text(selectedFood.servings == floor(selectedFood.servings) ? "\(Int(selectedFood.servings))x" : String(format: "%.1fx", selectedFood.servings)).font(.system(size: 13, weight: .black)).foregroundColor(.white)
                    Text(selectedFood.food.servingUnit).font(.system(size: 8)).foregroundColor(.white.opacity(0.4)).lineLimit(1)
                }.frame(minWidth: 44)
                Button(action: { onServingsChanged(selectedFood.servings + 0.5) }) { Image(systemName: "plus.circle.fill").font(.system(size: 20)).foregroundColor(.tmGold.opacity(0.8)) }
                Button(action: onRemove) { Image(systemName: "xmark.circle.fill").font(.system(size: 18)).foregroundColor(.red.opacity(0.6)) }
            }
        }
        .padding(10).background(RoundedRectangle(cornerRadius: 12).fill(Color.white.opacity(0.05))).overlay(RoundedRectangle(cornerRadius: 12).stroke(Color.white.opacity(0.07), lineWidth: 1))
    }
}
...
struct FoodLibraryBrowser: View {
    let mealTypeFilter: String; let onAdd: (FoodLibraryItem) -> Void
    @Environment(\.dismiss) var dismiss
    @State private var searchText = ""; @State private var selectedCategory: FoodLibraryItem.FoodCategory? = nil
    @State private var showSuggested = true; @State private var addedIds: Set<UUID> = []
    @State private var showingCustomEntry = false

    private var filtered: [FoodLibraryItem] {
        var list = showSuggested && searchText.isEmpty && selectedCategory == nil ? FoodLibrary.forMealType(mealTypeFilter) : selectedCategory != nil ? FoodLibrary.byCategory(selectedCategory!) : FoodLibrary.all
    ...
    var body: some View {
        NavigationView {
            ZStack {
                Color.black.ignoresSafeArea()
                VStack(spacing: 0) {
                    HStack(spacing: 10) { Image(systemName: "magnifyingglass").foregroundColor(.white.opacity(0.4)); TextField("Search foods...", text: $searchText).foregroundColor(.white).onChange(of: searchText) { _, _ in showSuggested = searchText.isEmpty } }
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 8) {
                            Button(action: { selectedCategory = nil; showSuggested = true }) {
                                HStack(spacing: 5) { Image(systemName: "star.fill").font(.system(size: 11)); Text("Suggested").font(.system(size: 12, weight: .semibold)) }
                                    .foregroundColor(showSuggested && selectedCategory == nil ? .black : .white.opacity(0.6)).padding(.horizontal, 12).padding(.vertical, 7).background(Capsule().fill(showSuggested && selectedCategory == nil ? Color.tmGold : Color.white.opacity(0.08)))
                            }.buttonStyle(.plain)
                            Button(action: { selectedCategory = nil; showSuggested = false }) {
                                Text("All").font(.system(size: 12, weight: .semibold)).foregroundColor(!showSuggested && selectedCategory == nil ? .black : .white.opacity(0.6)).padding(.horizontal, 12).padding(.vertical, 7).background(Capsule().fill(!showSuggested && selectedCategory == nil ? Color.tmGold : Color.white.opacity(0.08)))
                            }.buttonStyle(.plain)
                            ForEach(FoodLibraryItem.FoodCategory.allCases, id: \.self) { cat in
                                Button(action: { selectedCategory = cat; showSuggested = false }) {
                                    HStack(spacing: 5) { Image(systemName: cat.icon).font(.system(size: 11)); Text(cat.rawValue).font(.system(size: 12, weight: .semibold)) }
                                        .foregroundColor(selectedCategory == cat ? .black : .white.opacity(0.6)).padding(.horizontal, 12).padding(.vertical, 7).background(Capsule().fill(selectedCategory == cat ? cat.color : Color.white.opacity(0.08)))
                                }.buttonStyle(.plain)
                            }
                    ...
                    List {
                        ForEach(filtered) { food in
                            FoodLibraryRow(food: food, isAdded: addedIds.contains(food.id)) { onAdd(food); addedIds.insert(food.id) }
            ...
            .toolbarBackground(Color.black, for: .navigationBar).toolbarBackground(.visible, for: .navigationBar).toolbarColorScheme(.dark, for: .navigationBar)
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) { Button(action: { showingCustomEntry = true }) { HStack(spacing: 4) { Image(systemName: "plus.circle"); Text("Custom") }.font(.system(size: 13, weight: .semibold)).foregroundColor(.tmGold) } }
                ToolbarItem(placement: .navigationBarTrailing) { Button("Done") { dismiss() }.foregroundColor(.tmGold) }
            }
            .sheet(isPresented: $showingCustomEntry) { CustomFoodEntrySheet { food in onAdd(food); addedIds.insert(food.id); showingCustomEntry = false } }
        }
    }
}
...
struct FoodLibraryRow: View {
    let food: FoodLibraryItem; let isAdded: Bool; let onAdd: () -> Void
    var body: some View {
        HStack(spacing: 12) {
            ZStack { RoundedRectangle(cornerRadius: 8).fill(food.category.color.opacity(0.15)).frame(width: 40, height: 40); Image(systemName: food.category.icon).font(.system(size: 16)).foregroundColor(food.category.color) }
            VStack(alignment: .leading, spacing: 3) {
                Text(food.name).font(.system(size: 14, weight: .semibold)).foregroundColor(.white)
                HStack(spacing: 6) { Text(food.servingUnit).font(.caption).foregroundColor(.white.opacity(0.4)); Text("•").foregroundColor(.white.opacity(0.2)); Text("\(food.caloriesPerServing) cal").font(.caption).foregroundColor(.tmGold); Text("•").foregroundColor(.white.opacity(0.2)); Text("P:\(String(format: "%.0f", food.proteinPerServing))g").font(.caption).foregroundColor(.red.opacity(0.8)) }
            }
            Spacer()
...
struct CustomFoodEntrySheet: View {
    ...
    @Environment(\.dismiss) var dismiss
    @State private var name = ""; @State private var servingUnit = "1 serving"
    @State private var calories = ""; @State private var protein = ""; @State private var carbs = ""; @State private var fat = ""
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        fieldBlock("FOOD NAME") { TextField("e.g. Homemade Protein Pancakes", text: $name).foregroundColor(.white).padding(12).background(RoundedRectangle(cornerRadius: 10).fill(Color.white.opacity(0.06))).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.tmGold.opacity(0.3), lineWidth: 1)) }
                        fieldBlock("SERVING SIZE") { TextField("e.g. 1 cup, 4 oz", text: $servingUnit).foregroundColor(.white).padding(12).background(RoundedRectangle(cornerRadius: 10).fill(Color.white.opacity(0.06))) }
                        fieldBlock("CALORIES PER SERVING *") { TextField("e.g. 250", text: $calories).keyboardType(.numberPad).foregroundColor(.white).padding(12).background(RoundedRectangle(cornerRadius: 10).fill(Color.white.opacity(0.06))).overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.tmGold.opacity(0.3), lineWidth: 1)) }
                        HStack(spacing: 12) {
                            fieldBlock("PROTEIN (g)") { TextField("0", text: $protein).keyboardType(.decimalPad).foregroundColor(.white).padding(12).background(RoundedRectangle(cornerRadius: 10).fill(Color.red.opacity(0.08))) }
                            fieldBlock("CARBS (g)")   { TextField("0", text: $carbs).keyboardType(.decimalPad).foregroundColor(.white).padding(12).background(RoundedRectangle(cornerRadius: 10).fill(Color.blue.opacity(0.08))) }
                            fieldBlock("FAT (g)")     { TextField("0", text: $fat).keyboardType(.decimalPad).foregroundColor(.white).padding(12).background(RoundedRectangle(cornerRadius: 10).fill(Color.yellow.opacity(0.08))) }
                        }
                        Button(action: {
                            let item = FoodLibraryItem(name: name, category: category, servingSize: 100, servingUnit: servingUnit.isEmpty ? "1 serving" : servingUnit, caloriesPerServing: Int(calories) ?? 0, proteinPerServing: Double(protein) ?? 0, carbsPerServing: Double(carbs) ?? 0, fatPerServing: Double(fat) ?? 0, mealTypes: [])
                            onAdd(item); dismiss()
                        }) {
                            HStack(spacing: 8) { Image(systemName: "plus.circle.fill"); Text("ADD TO MEAL").font(.system(size: 15, weight: .heavy)).tracking(0.5) }
                                .foregroundColor(canAdd ? .black : .white.opacity(0.4)).frame(maxWidth: .infinity).frame(height: 52).background(RoundedRectangle(cornerRadius: 26).fill(canAdd ? Color.tmGold : Color.white.opacity(0.08)))
                        }.disabled(!canAdd)
                    }.padding(16)
                }
            }
